...
export default async function docxtopdf(req, res) {
  ...
  try {
    ...
    const message =
      'We are currently experiencing some issues with processing DOCX files. Please upload the PDF version or contact helpdesk@lhinigeria.org.';

    if (msg.includes('timed out')) {
      msg =
        'Document conversion is taking too long. Please try a smaller file or contact helpdesk@lhinigeria.org.';
    } else if (msg.includes('too large') || msg.includes('size')) {
      msg =
        'File is too large to process. Please reduce the file size or contact helpdesk@lhinigeria.org.';
    } else {
      msg = message;
    }
