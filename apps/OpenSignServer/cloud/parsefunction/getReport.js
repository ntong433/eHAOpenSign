...
export default async function getReport(request) {
  ...
  const signatureTraceId = request.params.signatureTraceId || request.headers['x-signature-trace-id'] || '';
  ...
  try {
    ...
    const userEmail = (userRes.data?.email || userRes.data?.username || '').toLowerCase().trim();
    const userIdentity = {
      userId,
      email: userRes.data?.email,
      username: userRes.data?.username,
      userPrincipalName: userRes.data?.userPrincipalName || userRes.data?.UPN,
      microsoftOid:
        userRes.data?.microsoftObjectId ||
        userRes.data?.microsoftOid ||
        userRes.data?.oid,
      emails: [
        userRes.data?.email,
        ...
        userRes.data?.Email,
        userRes.data?.UPN,
        userRes.data?.userPrincipalName,
      ],
    };
    if (!userId) {
    ...
    if (reportId === '4Hhwbp482K') {
      console.log(`[getReport] Report: Needs Your Signature`);
      console.log(`[getReport] Current User ID: ${userId}`);
      console.log(`[getReport] Current User Email: ${userEmail}`);
    }

    const json = reportId && reportJson(reportId, userId, userEmail, userIdentity);
    if (json) {
      ...
      const include =
        'AuditTrail.UserPtr,AuditTrail.UserPtr.UserId,AuditTrail.UserPtr.parseUser,Placeholders.signerPtr,Placeholders.signerPtr.UserId,Placeholders.signerPtr.parseUser,ExtUserPtr.TenantId,Signers,Signers.UserId,Signers.parseUser';
      ...
      if (reportId === '4Hhwbp482K') {
        console.log(
          '[signatureTrace:getReport:query]',
          JSON.stringify({
            signatureTraceId,
            currentUser: {
              objectId: userId,
              ...
              microsoftOid: userIdentity.microsoftOid || '',
            },
            className: clsName,
            where: paramsObj,
            include,
      ...
      if (res.data && res.data.results) {
        if (reportId === '4Hhwbp482K') {
          console.log(`[getReport] Documents returned for Needs Your Signature: ${res.data.results.length}`);
          console.log(
            '[signatureTrace:getReport:result]',
            JSON.stringify({
              signatureTraceId,
              resultCount: res.data.results.length,
              resultIds: res.data.results.map(doc => doc.objectId),
            })
          );
        }
        if (clsName === 'DirectoryUser') {
