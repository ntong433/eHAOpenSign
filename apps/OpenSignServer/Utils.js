...
export const cloudServerUrl = 'http://localhost:8085/app';
export const serverAppId = process.env.APP_ID || 'opensign';
export const appName = 'Life Helpers Signature Portal';
...
// SMTP config has been removed, all email is now routed via Microsoft Graph
...
export const mailTemplate = param => {
  const themeColor = '#ED3237';
  const subject = `Signature Requested — ${param.title}`;
  const AppName = appName;
  const logo = `<div style='font-family:system-ui;font-weight:700;color:#ED3237'>Life Helpers Initiative</div>`;
  ...
  const body =
    ...
    logo +
    `</div><div style='padding:2px;font-family:system-ui;background-color:${themeColor}'><p style='font-size:20px;font-weight:400;color:white;padding-left:20px'>Signature Requested</p></div><div><p style='padding:20px;font-size:14px;margin-bottom:10px'>` +
    param.senderName +
