...
export const defaultMailSubject = `Signature Requested — {{document_title}}`;
...
export const signPdfFun = async (
  ...
  try {
    ...
    const passthroughMessages = [
      ...
      "This signature request belongs to another account.",
      "This authorization session has expired. Verify your email again.",
      "This verification code has expired.",
      "This document has already been signed.",
      "You have already completed this signature.",
      "It is not yet your turn to sign.",
      "Document is no longer available for signing.",
    ...
    } else if (passthroughMessages.includes(e?.message)) {
      return { status: "error", message: e.message };
    } else {
      return { status: "error", message: "something went wrong." };
    }
...
export const embedDocId = async (pdfOriginalWH, pdfDoc, documentId) => {
  const appName = "Life Helpers Signature Portal";
...
export const embedWidgetsToDoc = async (
  ...
  if (!hasError) {
    ...
    return {
      error:
        "This PDF is not compatible with the Life Helpers Signature Portal. Please contact helpdesk@lhinigeria.org."
    };
  }
};
...
export const contractDocument = async (
  ...
  const data = {
    ...
    ...(externalSigningGrant ? { externalSigningGrant } : {}),
    ...(signerId ? { signerId } : {})
  };
  ...
  const documentDeatils = await axios
    ...
    .then((Listdata) => {
      ...
      console.info("GET_DOCUMENT_RESULT", {
    ...
    .catch((err) => {
      console.error("getDocument failed", {
        code: err?.code,
        message: err?.message
      });
      throw err;
...
export const getAppLogo = async () => {
  ...
  try {
    ...
    if (tenant) {
      ...
      localStorage.setItem("appname", "Life Helpers Signature Portal");
...
export const mailTemplate = (param) => {
  const appName = "Life Helpers Signature Portal";
  const logo = `<div style='padding:10px;font-family:system-ui;font-weight:700;color:#ED3237'>Life Helpers Initiative</div>`;

  const subject = `Signature Requested — ${param.title}`;
  const body =
    ...
    logo +
    `<div style='padding:2px;font-family:system-ui;background-color:#ED3237'><p style='font-size:20px;font-weight:400;color:white;padding-left:20px'>Signature Requested</p></div><div><p style='padding:20px;font-size:14px;margin-bottom:10px'>` +
    param.senderName +
    ...
    param.signingUrl +
    "><button style='padding:12px;background-color:#ED3237;color:white;border:0px;font-weight:bold;margin-top:30px'>Sign here</button></a></div><div style='display:flex;justify-content:center;margin-top:10px'></div></div></div><div><p> This is an automated email from " +
    appName +
    ". For assistance, contact helpdesk@lhinigeria.org.</p></div></div></body></html> ";
  return { subject, body };
};
...
export const getSignerEmail = (data, signers) => {
  const signer =
    signers?.length > 0 &&
    signers.find((x) => x.objectId === data.signerObjId);
  return (
    signer?.Email ||
    signer?.email ||
    signer?.mail ||
