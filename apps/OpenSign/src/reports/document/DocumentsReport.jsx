...
import {
  ...
  getSignerName as getRecipientName
...
const DocumentsReport = (props) => {
      ...
      if (x?.signerObjId) {
        const encodeBase64 = btoa(
          `${item.objectId}/${getSignerEmail(x, item?.Signers)}/${x?.signerObjId}/${sendMail}`
	        ...
	        return `${host}/login/${encodeURIComponent(encodeBase64)}`;
      } else {
  ...
  const handleSubjectChange = (subject, doc) => {
    ...
    const localExpireDate = newDate.toLocaleDateString("en-US", {
      ...
      year: "numeric"
	    ...
	    const signPdf = `${window.location.origin}/login/${encodeURIComponent(encodeBase64)}`;
    const variables = {
  ...
  const handleNextBtn = (user, doc) => {
    const recipientEmail = user.email || getRecipientEmail(user?.signerPtr);
    const userdata = {
      Name: getRecipientName(user?.signerPtr),
    ...
    const encodeBase64 = user.email
      ...
      : btoa(
          `${doc.objectId}/${recipientEmail}/${user.signerPtr.objectId}`
        );
    const expireDate = doc.ExpiryDate.iso;
    const newDate = new Date(expireDate);
    const localExpireDate = newDate.toLocaleDateString("en-US", {
    ...
    const variables = {
      ...
      sender_mail: doc?.SenderMail || doc.ExtUserPtr.Email,
      sender_phone: doc.ExtUserPtr?.Phone || "",
      receiver_name: getRecipientName(user?.signerPtr),
  ...
  const fetchUserStatus = (user, doc) => {
    const email = user.email || getRecipientEmail(user.signerPtr);
    const audit = doc?.AuditTrail?.find((x) => getRecipientEmail(x.UserPtr) === email);
  ...
  const handleAddUser = (data, id) => {
    const signerPtr = {
      __type: "Pointer",
      className: data.className || "contracts_Contactbook",
    ...
    const updateSigner = signerList.map((y) => {
      ...
      //condition is used to add new signer's mail to role
        ...
        return { ...y, ...data, className: data.className || "contracts_Contactbook" };
      }
      return { ...y };
    });
                                  ...
                                  {Object?.keys(isNextStep) <= 0 && (
                                    <div className="flex justify-between items-center gap-2 my-2 px-3">
                                      <div className="text-base-content">
                                        {getRecipientName(user?.signerPtr) || "-"}{" "}
                                        {`<${
                                          user?.email
                                            ? user.email
                                            : getRecipientEmail(user.signerPtr)
                                        }>`}
                                      </div>
