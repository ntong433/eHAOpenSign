      ...
      try {
        ...
        if (str.includes("#")) {
          ...
          if (HashCount.length > 1) {
            ...
            output = output.split(".");
            if (output.length > 1 && json[output[0]] !== undefined) {
              test1 = str.replace(reg1, json[output[0]][output[1]]);
            } else if (json[output[0]] !== undefined) {
            ...
            .then((res) => {
              ...
              for (const obj of listData) {
                const currentSigner = findSignerForCurrentUser(
                  obj.Signers,
                  currentUser?.toJSON?.() || currentUser
                );
                if (currentSigner) {
                  let isRecord;
                  if (obj?.AuditTrail && obj?.AuditTrail.length > 0) {
                    isRecord = obj?.AuditTrail.some(
                      (item) =>
                        auditBelongsToSigner(item, currentSigner.objectId) &&
          ...
          try {
            ...
            if (output.length === 1) {
              ...
              if (output.length === 1) {
                ...
                output = output.substring(1, output.length - 1);
                output = output.split(".");
                if (output.length > 0 && json[output[0]] !== undefined) {
