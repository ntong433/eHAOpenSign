...
const SignersInput = (props) => {
  ...
  const toSignerOption = (item = {}) => ({
    ...item,
    value: item?.objectId || item?.value,
  ...
  useEffect(() => {
    ...
    if (props?.initialData && props?.initialData?.length > 0) {
      const trimEmail = props?.initialData.map((item) => toSignerOption(item));
  ...
  useEffect(() => {
    if (Array.isArray(selected)) {
      const newData = selected.map((x) =>
        props?.isCaptureAllData ? x : x.value
      );
      ...
      if (contactRes) {
        ...
        const compareArrays = (res, signerObj) => {
          const selectedIds = new Set(
            (signerObj || [])
              .map((item) => `${item.className || ""}:${item.objectId || ""}`)
              .filter(Boolean)
          );
          return res.filter(
            (item1) => !selectedIds.has(`${item1.className || ""}:${item1.objectId || ""}`)
          );
        };
