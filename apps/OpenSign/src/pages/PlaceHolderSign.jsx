...
function PlaceHolderSign() {
  ...
  const getDocumentDetails = async () => {
    ...
    if (documentData && documentData.length > 0) {
      ...
      const persistedRecipients = mergeRecipients(
        location?.state?.selectedRecipients || [],
        readWizardRecipients(documentId)
      );
      if (persistedRecipients.length > 0) {
        updatedPdfDetails[0].Signers = mergeRecipients(
          persistedRecipients,
          updatedPdfDetails[0]?.Signers || []
        );
        documentData[0].Signers = updatedPdfDetails[0].Signers;
      }
      ...
      if (data && data.objectId) {
        ...
        const updateSigner = signersdata.map((x) => {
          if (x.Id === id || x.objectId === id) {
            return { ...x, ...data, className: data.className || "contracts_Contactbook" };
          }
          return { ...x };
        });
