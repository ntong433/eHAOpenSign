...
import {
  ...
  getSignerEmail as getRecipientEmail,
  getSignerName,
  pointerId
...
function PdfRequestFiles(
  ...
  sendmail = queryParams.get("sendmail") || undefined;
  signingToken = queryParams.get("token") || "";
  const externalGrantSession = readExternalSigningGrant(signingToken);
  const externalSigningGrant = externalGrantSession?.grantToken || "";
  const isExternalSigningSession = Boolean(externalSigningGrant);
    ...
    try {
      ...
      const contactId = jsonSender?.objectId
        ...
        : contactBookId ||
          signerObjectId ||
          "";
      const tenantSignTypes = localStorage.getItem("accesstoken")
        ? await fetchTenantDetails(contactId)
        : [];
      ...
      const documentData = await contractDocument(
        ...
        undefined,
        signingToken,
        externalSigningGrant,
        contactBookId
      );
      if (!Array.isArray(documentData) || !documentData[0]) {
      ...
      if (documentData && documentData.length > 0) {
        ...
        const getCurrentSigner = findSignerForCurrentUser(getSigners, jsonSender);
        ...
        if (audittrailData && audittrailData.length > 0) {
          ...
          for (const item of placeholdersOrSigners) {
            const checkEmail = getRecipientEmail(item);
            //if email exist then compare user signed by using email else signers objectId
            const emailOrId = checkEmail || item?.objectId;
            ...
            for (const doc of audittrailData) {
              const signedExist = checkEmail
                ? getRecipientEmail(doc?.UserPtr)
                : pointerId(doc?.UserPtr);
        ...
        if (
          ...
          } else if (isExternalSigningSession) {
            const currentSignerDetails = documentData[0]?.Signers?.find(
              (item) => item.objectId === currUserId
            );
            if (currentSignerDetails) {
              storeSignerDetails(currentSignerDetails);
              setSignerUserId(currUserId);
              setContractName(currentSignerDetails?.className || "_Contactbook");
            }
            setIsReqSignTourDisabled(true);
          } else if (!isEnableOTP) {
            try {
  ...
  // `storeSignerDetails` is used to stor signer details to set in text type widgets
	  const storeSignerDetails = (signer) => {
	    localStorage.setItem(
	      "signer",
	      JSON.stringify({
	        Name: getSignerName(signer),
	        Email: getRecipientEmail(signer),
  ...
  async function embedWidgetsData(
    ...
    let currentUser = localuser ? JSON.parse(localuser) : null;
    let isEmailVerified = isExternalSigningSession ? true : currentUser?.emailVerified;
                ...
                const resSign = await signPdfFun(
                  ...
                  externalSigningGrant
	                ...
	                if (resSign && resSign.status === "success") {
	                  if (isExternalSigningSession) {
	                    if (!resSign.success || !resSign.completionReference) {
	                      throw new Error(
	                        "The signed document download authorization was not returned."
	                      );
	                    }
	                    clearExternalSigningGrant(signingToken);
	                    window.location.href = `/external-signing/completed/${encodeURIComponent(
	                      resSign.completionReference
	                    )}`;
	                    return;
	                  }
                  ...
                  const isSuccessRoute = pdfDetails?.[0]?.RedirectUrl
                    ...
                    : window.location?.pathname?.includes("load");
	                  const updatedDoc = Array.isArray(resSign.updatedPdfDetails) &&
	                    resSign.updatedPdfDetails.length > 0
	                    ? { updatedPdfDetails: resSign.updatedPdfDetails }
	                  ...
	                  if (!updatedDoc?.updatedPdfDetails?.[0]) {
	                    throw new Error("Updated PDF details were not returned.");
	                  }
	                  const index =
                  ...
                  const nextSignerEmail =
                    nextPlaceholder?.email || getRecipientEmail(nextSigner);
                  const user = nextSignerEmail
                    ? {
                        ...nextSigner,
                        objectId: nextSigner?.objectId || nextPlaceholder?.signerObjId,
                        Name: getSignerName(nextSigner),
                  ...
                  if (
                    ...
                    if (user) {
                      ...
                      try {
                        ...
                        const userEmail = getRecipientEmail(user);
                        const hostUrl = window.location.origin;
                        //encode this url value `${pdfDetails?.[0].objectId}/${userEmail}/${objectId}` to base64 using `btoa` function
                        let encodeBase64;
                        if (objectId) {
                          encodeBase64 = btoa(
                            `${docId}/${userEmail}/${objectId}`
                          );
                        } else {
                          encodeBase64 = btoa(`${docId}/${userEmail}`);
                        }
	                        let signPdf =
	                          `${hostUrl}/login/${encodeURIComponent(encodeBase64)}`;
 ...
 const pageDetails = async (pdf) => {
  ...
  const closeRequestSignTour = async () => {
    ...
    if (isDontShow) {
      const isEnableOTP = pdfDetails?.[0]?.IsEnableOTP || false;
      const sessionToken = localStorage.getItem("accesstoken");
      if (!isEnableOTP && !sessionToken && !isExternalSigningSession) {
        try {
          await axios.post(
