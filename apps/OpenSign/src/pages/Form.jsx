...
import {
  dedupeRecipients,
  normalizeRecipient,
  storeWizardRecipients
} from "../utils/recipientPersistence";
...
const Forms = (props) => {
  ...
  const [selectedSignerDetails, setSelectedSignerDetails] = useState([]);
  ...
  const handleSubmit = withSessionValidation(async (e) => {
    ...
    if (fileupload) {
      ...
      try {
        ...
        if (res) {
          const persistedRecipients = selectedSignerDetails;
          storeWizardRecipients(res.id, persistedRecipients);
          setSigners([]);
          setSelectedSignerDetails([]);
          setBcc([]);
          setCc([]);
          ...
          navigate(`/${props?.redirectRoute}/${res.id}`, {
            state: { selectedRecipients: persistedRecipients }
          });
        }
      } catch (err) {
  ...
  const handleSigners = (data) => {
    if (data && data.length > 0) {
      const canonical = dedupeRecipients(
        data.map((x, index) =>
          typeof x === "string"
            ? normalizeRecipient({ objectId: x }, index)
            : normalizeRecipient(
                {
                  ...x,
                  objectId: x.objectId || x.value,
                  Name: x.Name || x.label,
      ...
      const updateSigners = canonical.map((x) => ({
        __type: "Pointer",
        className: x.className || "contracts_Contactbook",
        objectId: x.objectId
      }));
      setSelectedSignerDetails(canonical);
